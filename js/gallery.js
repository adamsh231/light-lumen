/**
 * In Common With — All Lighting Collection (Full-Bleed Mosaic, Hover Flip, Master Switch)
 */
(function (root, factory) {
  if (typeof module === 'object' && module.exports) {
    module.exports = factory();
  } else {
    root.AppGallery = factory();
  }
  ...
  function toggleMasterGallerySwitch() {
    const { catalog, state } = getStore();
  ...
  function openLampInStudio(lampId) {
    const { catalog } = getStore();
    const idx = catalog.findIndex(l => l.id === lampId);
    if (idx !== -1) {
